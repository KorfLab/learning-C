...
1. `git clone https://github.com/KorfLab/learning-C`
2. Make a directory for yourself inside the `students` directory.

## Hello World ##

Open your favorite editor and create the following program: `hello_world.c`.
You can find this program in the `templates` directory of the repository in
...
by `git` because it's in the `.gitignore` file. Here's how I would go about
...
recognize single-line comments, which were introduced in C++ (maybe).
...
In C, all variables are strictly typed. You cannot create a variable and decide
later if it will contain an integer, floating point number, or string.

Copy the `variables.c` program from `templates` to your directory. Compile and
run that. Then add your own variables and your own `printf()` statements.
...
of your variables equal to zero at the start of your program? Absolutely not. A
...
Copy `templates/strings.c` to your directory. Read the text below, then compile
...
developing on multiple platforms makes software more robust. I am currently
using Mac OS, Debian, Linux Mint, and Haiku.
...
Copy `templates/conditionals.c` to your directory and try that out. Most of it
should look pretty familiar to you. Python doesn't have `case`, `do`, or `goto`
...
rarely see `obj.method()` (although I do this on occasion).
...
from the `templates` directory and play with that.


## Memory ##

Everything up to now should have been mostly familiar. But we're about to go
off the chart and there be dragons out there...
...
Grab `templates/memory.c` and try it out. Make sure you have `top` or some
...
goes out of scope, and the memory that was originally created in `f2()` now
gets garbage collected. In Python, every piece of memory has a "reference
count". In `f1()`, when `a` gets associated with the memory of `hello`, the
memory has a reference count of 1. When execution leaves the function, the
count goes to 0 and the memory is freed. In `f2()` the return statement
increases the reference count by 1. So even though `a` goes out of scope,
reducing the reference count by 1, the memory still has a reference count of 1.
When `thing` finally goes out of scope, the reference count goes to zero and
...
address? It depends if you are using a 32-bit or 64-bit OS. So, either 4 or 8
bytes.
